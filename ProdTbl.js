...
function addItemsToTblRow(pId, pName, pCateg, pWeight,pQuantity, pPrice, pSale,pUpdatedSalePrice, pBrand,pSupp, pblock, pRack, pShelf)
{
    ...
    let tc13= document.createElement('td');

    dataList.push([pId,pName,pCateg,pWeight,pQuantity,pPrice,pSale,pUpdatedSalePrice,pBrand,pSupp,pblock,pRack,pShelf]);
    ...
    tc7.innerHTML=pSale;
    tc8.innerHTML=pUpdatedSalePrice;
    tc9.innerHTML=pBrand;
    tc10.innerHTML=pSupp;
    tc11.innerHTML=pblock;
    tc12.innerHTML=pRack;
    tc13.innerHTML=pShelf;
    ...
    tc7.classList+="psale";
    tc8.classList+="pUpdatedPrice";
    tc9.classList+="pbrand";
    tc10.classList+="psupp";
    tc11.classList+="pblock";
    tc12.classList+="prack";
    tc13.classList+="pshelf";
...
function addAllItems(Alldata){
    ...
    Alldata.forEach(element => {
        addItemsToTblRow(element.pId,element.pName,element.Pcategory,element.weight,element.pQuantity,element.Pprice,element.PsalePercent,element.pUpdatedSalePrice,element.Pbrand,element.supplierId,element.PblockId,element.PrackNo,element.PshelfNo);
    });
}
...
function searchItemsByExactValue(Category) //This function search data by exact value
{
    ...
    for (let i = 0; i < tr.length; i++)
    {
        var td=tr[i].getElementsByClassName(Category); // Searching every row element of the specified category
        for (let j = 0; j < td.length; j++) {
...
window.FillData=(index)=>       //This function shows data in Product Model section
{
   ...
   else
   {
    ...
    --index;
        ...
        updatedPrice.value = dataList[index][7];
        brand.value = dataList[index][8];
        spId.value = dataList[index][9];
        block.value = dataList[index][10];
        rack.value = dataList[index][11];
        shelf.value = dataList[index][12];
...
{
    if(sale.value>0){
    updatedPrice.value=price.value-((sale.value/100)*price.value);
    ...
    update(ref(db, "StoreDatabase/"+id.value),{
        ...
        pUpdatedSalePrice : Number(updatedPrice.value),
        Pbrand: brand.value,
